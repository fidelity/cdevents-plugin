...
@SuppressFBWarnings(
        value = {"RCN_REDUNDANT_NULLCHECK_WOULD_HAVE_BEEN_A_NPE", "RV_RETURN_VALUE_IGNORED_NO_SIDE_EFFECT"},
...
public class EventHandlerTest {
    ...
    void testpipelineRunQueuedEvent() {
        try (MockedStatic<BuildCDEvent> mockBuildCDEvent = mockStatic(BuildCDEvent.class);
                MockedStatic<CDEventsGlobalConfig> mockCDEventsGlobalConfig =
                        mockStatic(CDEventsGlobalConfig.class, Answers.RETURNS_DEEP_STUBS)) {
            CloudEvent mockCloudEvent = mock(CloudEvent.class);
            Queue.WaitingItem mockItem = mock(Queue.WaitingItem.class);
            mockBuildCDEvent
                    .when(() -> BuildCDEvent.buildPipelineRunQueuedModel(any()))
                    .thenReturn(mockCloudEvent);
            mockCDEventsGlobalConfig
                    .when(() -> CDEventsGlobalConfig.get().getSinkType())
                    .thenReturn("syslog");
